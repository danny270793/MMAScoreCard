...
export class Sherdog {
    ...
    async getFightsFromEvent(event: Event): Promise<Fight[]> {
        ...
        for (const table of tables) {
            ...
            for (const row of rows) {
                ...
                if (cells.length === 5) {
                    ...
                    const [decision, rawMethod] = cells[4]
                        .textContent!.trim()
                        .split('(')
                    ...
                    const method = rawMethod?.replace(')', '').trim()
                    const resultOne =
        ...
        for (const resume of resumes) {
            const leftSide = (resume as HTMLTableRowElement).querySelector('div.fighter.left_side')!
            const rightSide = (resume as HTMLTableRowElement).querySelector('div.fighter.right_side')!
            ...
            const titleFight = !!(resume as HTMLTableRowElement).querySelector('span.title_fight')

            const span = (resume as HTMLTableRowElement).querySelector('span.weight_class')
            ...
            if (!table) {
                    ...
                    category:
                        categoryParts.length > 1
                            ? {
                                  name: categoryParts[1].trim(),
                                  weight: parseInt(
                                      categoryParts[0].replace('lb', '').trim(),
                                  ),
                              }
                            : {
                                  name: categoryParts[0].trim(),
                ...
                const resultOne =
                    leftSide.textContent!.split('\n').pop()?.trim() || ''
                const resultTwo =
                    rightSide.textContent!.split('\n').pop()?.trim() || ''

                for (const row of table.querySelectorAll('tr')) {
                    const cells = row.querySelectorAll('td')
                    const [decisionRaw, methodRaw] =
                        cells[1].textContent!.split('(')
                    const referee = cells[2]
                        .textContent!.replace('Referee ', '')
                        .trim()
    ...
    async getStatsFighter(fighter: Fighter): Promise<Stats> {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            if (index === 0) {
                ...
                if (rows.length === 3) {
                    height =
                        cellText.split('/')[1]?.replace('cm', '').trim() ?? null
                } else {
                ...
                if (rows.length === 3) {
                    weight =
                        cellText.split('/')[1]?.replace('kg', '').trim() ?? null
                } else {
                    height =
                        cellText.split('/')[1]?.replace('cm', '').trim() ?? null
                }
    ...
    async getFightsFromFighter(fighter: Fighter): Promise<NoEventFight[]> {
        ...
        for (
            ...
            const rows = Array.from((table as HTMLTableRowElement).querySelectorAll('tr')).slice(1)
