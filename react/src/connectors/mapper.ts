...
export const mapper = {
    ...
    toCity(city: BackendCity, countries: BackendCountry[]): City {
        return {
            id: city.id,
            name: city.name,
            country: mapper.toCountry(countries.find(country => country.id === city.countryId)!),
        };
    },
    toLocation(location: BackendLocation, cities: BackendCity[], countries: BackendCountry[]): Location {
        return {
            id: location.id,
            name: location.name,
            city: mapper.toCity(cities.find(city => city.id === location.cityId)!, countries),
        };
    },
    toEvent(event: BackendEvent, locations: BackendLocation[], cities: BackendCity[], countries: BackendCountry[]): Event {
        return {
            id: event.id,
            name: event.name,
            fight: event.fight,
